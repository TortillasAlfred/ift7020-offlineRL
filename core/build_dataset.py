...
class DatasetBuilder:
    ...
    def __init__(self, collection, train_val_split=0.8):
        self.collection = collection
        self.train_val_split = train_val_split

    def build_dataset(self):
        train_trajectories = self.collection[:int(0.8 * len(self.collection))]
        valid_trajectories = self.collection[int(0.8 * len(self.collection)):]

        train_data = GraphDataset(train_trajectories)
        train_loader = torch_geometric.data.DataLoader(train_data, batch_size=32, shuffle=True)
        valid_data = GraphDataset(valid_trajectories)
